...
function getFullName() {
    ...
    buttonSubmit.addEventListener('click', () => {
        ...
        let userFName = document.querySelector('#userFName').value;
        let userSName = document.querySelector('#userSName').value;
    
        const user = {
               
            get full() {
                if(userFName.length <= 6 || userSName.length <= 6) {
                    return 'Имя или фамилия должны содержать не менее 6 символов';
                } else {
                    return `${userFName} ${userSName}`;
                }
            },
    
            set full(names) {
                [userFName, userSName] = names.split(' ');
            }
