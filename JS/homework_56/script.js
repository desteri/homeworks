...
async function axiosServer() {
    ...
    return await axios('http://localhost:3000/posts');
    
}

axiosServer().then(res => {
    ...
    for (let i = 0; i < res.data.length; i++) {
        ...
        post.innerHTML += `<div class="post__item">
                                <div class="post__title red">${res.data[i].title}</div>
                                <div class="post__desc">${res.data[i].description}</div>
                            </div>
        `;
...
function localSt() {
    ...
    const toDoMon = localStorage.setItem('Понедельник', 'Занятия');
    const toDoTue = localStorage.setItem('Вторник', 'Курсы');
    const toDoWed = localStorage.setItem('Среда', 'Хобби');
    const toDoThu = localStorage.setItem('Четверг', 'Работа');
    const toDoFri = localStorage.setItem('Пятница', 'Отдых');
    const toDoSat = localStorage.setItem('Суббота', 'Дела');
    const toDoSun = localStorage.setItem('Воскресенье', 'Уборка');

    for(let i =  0; i < localStorage.length; i++) {

        let key = localStorage.key(i);
        lstUl.innerHTML += `<li>${key}: ${localStorage.getItem(key)}</li>
        `;

    }

    const listLocal = document.querySelector('.localList');
    listLocal.appendChild(lstUl);
